...
const logExecutionTime = async (operation, collection, dbOperation) => {
  ...
  try {
    ...
    console.log(`[DB] ${operation} trong '${collection}' từ [${callerInfo}] hoàn thành trong ${responseTime}ms`);
    return result;
  } catch (error) {
    const responseTime = Date.now() - startTime;
    console.error(`[DB ERROR] ${operation} trong '${collection}' từ [${callerInfo}] thất bại sau ${responseTime}ms:`, error);
    throw error;
  }
};
