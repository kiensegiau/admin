...
import { Card, Collapse, Button, Modal, Form, Input, message, Popconfirm, Table, Tooltip, Tag } from "antd";
import { doc, getDoc, updateDoc } from "firebase/firestore";
import { db } from "@/app/firebase";
import { PlusOutlined, EditOutlined, DeleteOutlined, EyeOutlined, LinkOutlined, PlayCircleOutlined, FileOutlined, FileImageOutlined } from '@ant-design/icons';
...
export default function EditCoursePage({ params }) {
    ...
    try {
      ...
      switch (modalType) {
        ...
        case 'add-file':
          ...
          const existingFile = selectedLesson.files?.find(f => f.url === validatedUrl);
          if (existingFile) {
  ...
  const handleDelete = async (type, chapterId, lessonId = null) => {
    try {
      ...
      if (type === 'chapter') {
        updatedChapters = updatedChapters.filter(chapter => chapter.id !== chapterId);
      } else if (type === 'lesson') {
        updatedChapters = updatedChapters.map(chapter => {
  ...
  const handleDeleteFile = async (chapterId, lessonId, driveFileId) => {
    try {
      ...
      const fileToDelete = lesson.files.find(f => (f.driveFileId || f.key) === driveFileId);
      if (!fileToDelete) {
        ...
        if (c.id === chapterId) {
          return {
            ...
            lessons: c.lessons.map(l => {
              if (l.id === lessonId) {
                const updatedFiles = l.files.filter(f => (f.driveFileId || f.key) !== driveFileId);
                return {
                  ...l,
                  files: updatedFiles
                };
  ...
  const renderFileList = (lesson, chapter) => {
    ...
    const dataSource = lesson.files.map(file => ({
      ...
      updateTime: file.updateTime || file.uploadTime || new Date().toISOString()
    }));
          ...
          return (
            <div className="flex gap-2">
              ...
              <Tooltip title="Sửa">
                <Button
                  icon={<EditOutlined />}
                  onClick={() => showFileModal('edit-file', chapter, lesson, file)}
                />
              </Tooltip>
              <Tooltip title="Xóa">
                <Popconfirm
  ...
  return (
    <div className="p-6">
                    ...
                    header={
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex gap-2" onClick={e => e.stopPropagation()}>
                          <Button 
                            icon={<EditOutlined />}
                            onClick={() => showModal('edit-lesson', chapter, lesson)}
                          >
                            Sửa
                          </Button>
                          <Popconfirm
                            title="Bạn có chắc chắn muốn xóa bài học này?"
                            onConfirm={() => handleDelete('lesson', chapter.id, lesson.id)}
                            okText="Có"
                            cancelText="Không"
                          >
                            <Button danger icon={<DeleteOutlined />}>
      ...
      <Modal
        title={
          modalType === 'chapter' ? 'Thêm chương mới' :
          modalType === 'lesson' ? 'Thêm bài học mới' :
          modalType === 'edit-chapter' ? 'Sửa tên chương' :
          modalType === 'edit-lesson' ? 'Sửa tên bài học' :
          modalType === 'add-file' ? 'Thêm file mới' :
          'Sửa thông tin file'
        }
        open={modalVisible}
        onOk={handleModalOk}
      ...
      >
        <Form form={form} layout="vertical">
          {['chapter', 'lesson', 'edit-chapter', 'edit-lesson'].includes(modalType) ? (
            <Form.Item
              name="title"
              label="Tên"
