...
async function getVideoId(did) {
    // Hàm retry với timeout
    const getVideoWithRetry = async (attempt = 1, maxAttempts = 3) => {
        try {
            ...
            if (match) {
                console.log('Found video ID, stopping search...');
                return match[1];
            }
            ...
            if (uplArrayMatch) {
                ...
                const video = uplArray.find(v => v.link?.includes('/index/'));
                if (video) {
                    const vidMatch = video.link.match(/\/index\/([a-f0-9]+)/);
                    if (vidMatch) {
                        console.log('Found video ID in upl_array, stopping search...');
                        return vidMatch[1];
                    }
                }
            }
            ...
            if (attempt < maxAttempts) {
                ...
                return getVideoWithRetry(attempt + 1, maxAttempts);
            }
            throw new Error('Could not find video ID after multiple attempts');
        } catch (error) {
            if (attempt < maxAttempts) {
